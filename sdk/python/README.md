...
1. A `provider/` folder containing the building and implementation logic
   1. `cmd/`
      1. `pulumi-gen-foo/` - generates language SDKs from the schema
      2. `pulumi-resource-foo/` - holds the package schema, injects the package version, and starts the gRPC server
   2. `pkg`
...
The [JSON schema file](https://www.pulumi.com/docs/guides/pulumi-packages/schema) is used by `pulumi-gen-foo` to create language-specific SDKs. 
...
This repository provides the [foo example schema](./provider/cmd/pulumi-resource-foo/schema.json) to get you started.  
